...
export function TrustBar() {
  return (
    <div className="bg-white px-6 pt-2 pb-20 md:px-10">
      <Reveal
        trigger="inView"
        className="mx-auto grid max-w-7xl grid-cols-1 gap-8 divide-y divide-blue-950/10 sm:grid-cols-2 sm:gap-x-10 sm:gap-y-8 sm:divide-y-0 lg:grid-cols-4 lg:divide-x"
      >
        {TRUST_ITEMS.map((item) => (
          <RevealItem
            key={item.title}
            className="pt-8 first:pt-0 lg:px-6 lg:pt-0 lg:first:pl-0"
          >
            <TrustItem
              icon={item.icon}
              title={item.title}
